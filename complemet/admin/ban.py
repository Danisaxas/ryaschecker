...
async def ban_user(client: Client, message: types.Message):
    try:
        ...
        if len(args) < 2:
            await message.reply_text(lang_data['ban_usage'], reply_to_message_id=message.id)
            return
        ...
        try:
            ...
            await message.reply_text(lang_data['ban_validation'], reply_to_message_id=message.id)
            return
        ...
        if current_status.lower() == "baneado":
            await message.reply_text(lang_data.get('already_banned', "<b>El usuario ya está baneado.</b>"), reply_to_message_id=message.id)
            return
